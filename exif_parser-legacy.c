...
const char *get_error_string(ErrorCode code) {
  ...
  case ERR_ENDIAN_MISSING:
    return "Missing Enddianess";
  case ERR_TIFF_MISSING:
    return "Missing TIFF";
  case ERR_INVALID_TAG:
    return "Tag is unkown or invalid";
  case ERR_MALLOC_BYTE_TRANS:
    return "Error allocating memory when translating from byte";
  case ERR_MALLOC_ASCII_TRANS:
    return "Error allocating memory when translating from ascii";
  case ERR_MALLOC_SHORT_TRANS:
    return "Error allocating memory when translating from short";
  case ERR_MALLOC_LONG_TRANS:
    return "Error allocating memory when translating from long or signed";
  case ERR_SHORT_COUNT:
    return "The count of short items exceeds 1";
  case ERR_LONG_COUNT:
    return "The count of long items exceeds 4";
  case ERR_RATIONAL_COUNT:
    return "The count of rational items exceeds 1";
  case ERR_MALLOC_RATIONAL_TRANS:
    return "Error allocating memory when translating from rational or signed "
           "rational";
  case ERR_MALLOC_UNDEFINED_TRANS:
    return "Error allocating memory when translating from undefined value";
  case ERR_UNKNOWN:
...
/**
 * @brief Takes in a jpeg image as Uint8_t pointer and searches for the exif
 * chunk. once found will pass the chunk to another function for parsing.
 *
 * @param buffer Uint8_t pointer for the image
 * @param length Length of the image file
 */
void parse_jpeg(const uint8_t *buffer, size_t length, char *outputBuffer,
                size_t outputCap) {
  size_t i = 2;
  ...
  while (i + 4 < length) {
    ...
    if (((buffer[i] << 8) | buffer[i + 1]) == 0xFFE1) {
      ...
      if (i + 9 + seg_length <= length && buffer[i + 4] == 'E' &&
          buffer[i + 5] == 'x' && buffer[i + 6] == 'i' &&
          buffer[i + 7] == 'f' &&

          buffer[i + 8] == 0x00 && buffer[i + 9] == 0x00) {
        ...
        ErrorCode response =
            read_jpeg_u8(buffer, offset, seg_length, outputBuffer, outputCap);
 ...
 * @return uint8_t*
 */
static ErrorCode read_jpeg_u8(const uint8_t *buffer, size_t offset,
                              size_t exifLength, char *outputBuffer,
                              size_t outputCap) {
  // tracks the writting postions
  int pos = 0;
  outputBuffer[pos++] = '{';

  // Tracks how many tags their are
  uint16_t tiff_tags = 0;
  uint16_t exif_tags = 0;

  // Tracks the endianess
  ...
  // Itterates through the tiff_tags first then the exif_tags
  for (int j = 0; j < tiff_tags + exif_tags; j++) {
    ...
    // ** COUNT ** //
    const uint32_t count = ((buffer[i]) << 24 | (buffer[i + 1]) << 16 |
    ...
    case 0x0003: // short
      ...
      if (count > 1) {
        ...
        // Adding the itemd
        snprintf(&output[0], sizeof(output), "%d",
    ...
    case 0x0004:
      ...
      // ** EXIF OFFSET ** //
        ...
        i = offset + ((value[0] << 24) | (value[1] << 16) | (value[2] << 8) |
                      (value[3]));
      ...
      // ** LONG ** //
        ...
        // Adding the item
        snprintf(&output[0], sizeof(output), "%d",
                 (uint32_t)((value[0] << 24 | value[1] << 16 | value[2] << 8 |
                             value[3])));
        fflush(stdout);
        VPRINT("count: %s\n", output);
        status = ERR_OK;
    ...
    case 0x0009: // slong
      ...
      // Adding the item
      snprintf(&output[0], 15, "%d",
               (int32_t)((value[0] << 24 | value[1] << 16 | value[2] << 8 |
                          value[3])));
      fflush(stdout);
    ...
    if (status == ERR_OK) {
      ...
      if (strcmp(tagName, "unknown") && pos + valLen + tagLen < outputCap &&
          tag != 0x8769) {
  ...
  // printf("%s", outputBuffer);s
  return ERR_OK;
}

static ErrorCode translate_byte(const uint32_t count, const uint8_t *val_or_off,
                                size_t offset, const uint8_t *buffer,
                                char *output) {

  // fetches the value from the tag
  const uint32_t value = ((val_or_off[0]) << 24 | (val_or_off[1]) << 16 |
                          (val_or_off[2]) << 8 | (val_or_off[3]));
...
static ErrorCode translate_ascii(const uint32_t count,
                                 const uint8_t *val_or_off, size_t offset,
                                 const uint8_t *buffer, char *output) {
...
static ErrorCode translate_rational(const uint32_t count,
                                    const uint8_t *val_or_off, size_t offset,
                                    const uint8_t *buffer, char *output) {
  ...
  const uint32_t numerator =
      (buffer[dataLoc++] << 24 | buffer[dataLoc++] << 16 |
       buffer[dataLoc++] << 8 | buffer[dataLoc++]);
  ...
  snprintf(&output[0], 15, "%d/%d", numerator, denominator);
  fflush(stdout);
...
static ErrorCode translate_srational(const uint32_t count,
                                     const uint8_t *val_or_off, size_t offset,
                                     const uint8_t *buffer, char *output) {
  ...
  // Read the values from the resgister
  const int32_t numerator = (buffer[dataLoc++] << 24 | buffer[dataLoc++] << 16 |
                             buffer[dataLoc++] << 8 | buffer[dataLoc++]);
